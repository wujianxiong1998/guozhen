...
import {
    overHaulProgramService, deleteService
...
export default {
    ...
    effects: {
        ...
        * loadStaffTree({payload}, {call, put, select}) {
            const { userInfo } = yield select(state => state.overHaulDesign);
        ...
        * getEquipmentList({payload}, {call, put, select}) {
            const {searchParams} = yield select(state => state.overHaulDesign.equipmentSelect);
            let {
                newItem
            } = yield select(({overHaulDesign}) => overHaulDesign);
        ...
        *getList({ payload = {} }, { call, put, select }) {
           ...
           pageSize = 'pageSize' in payload ? payload.pageSize : pageSize;
            ...
            const { data } = yield call(overHaulProgramService.getList, params);
        ...
        *save({ payload = {} },{select,call,put}){
            ...
            const { newItem, structureList, structuresTotal, equipmentSelectTotal } = yield select( ({overHaulDesign}) => overHaulDesign );
            ...
            let { data } = yield call(overHaulProgramService.save, params);
            if(!!data && data.result == 0){
        ...
        *update({ payload = {} },{select,call,put}){
            ...
            let { data } = yield call(overHaulProgramService.update, params);
            if(!!data && data.result == 0){
                yield put({type:'getList'});
