...
export default {
    ...
    subscriptions: {
        setup({ dispatch, history }) {
            return history.listen(({ pathname, search }) => {
                if (pathname === '/techniqueChangeLog') {
                    // 初始化state
                    dispatch({
                        type : 'updateState',
                        payload : {
    ...
    effects: {
        ...
        *getList({ payload = {} }, { call, put, select }) {
            ...
            } = yield select(({techniqueChangeLog}) => techniqueChangeLog);
           ...
           pageSize = 'pageSize' in payload ? payload.pageSize : pageSize;
            ...
            const { data } = yield call(techniqueChangeRecordService.getList, params);
        ...
        *getD({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(techniqueChangeRecordService.detail, params);
            if(!!data && !data.result){
                yield put({
                    type: 'updateState',
                    payload: {
