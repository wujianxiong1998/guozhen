import { getQuestionLibraryList,saveQuestionLibrary,deleteQuestionLibrary } from '../services/expertService';
import {
...
export default {
    ...
    effects : {
        ...
        *getList({ payload = {} }, { call, put, select }) {
            ...
            } = yield select(({questionLibrary}) => questionLibrary);
            ...
            const { data } = yield call(getQuestionLibraryList, VtxUtil.submitTrim(params));
        ...
        *saveOrUpdate({ payload }, { call, put, select }) {
            ...
            const { newItem, editItem } = yield select( ({questionLibrary}) => questionLibrary );
            const {
                id, title,
                content, reasonAnalysis, responses, businessId, type,
            } = payload.btnType === 'add' ? newItem : editItem;
            let params = {
                id, title, content, reasonAnalysis, responses, businessId, type,
                tenantId : VtxUtil.getUrlParam('tenantId')
            };
            const { data } = yield call( saveQuestionLibrary, VtxUtil.submitTrim(params));
        ...
        *deleteItems({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(deleteQuestionLibrary, params);
            if(!!data && data.result==0){
