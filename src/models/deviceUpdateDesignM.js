...
import {
    deviceUpdateProgramService, deleteService, wheatherAdminByUserIdService
...
export default {
    ...
    subscriptions: {
        setup({ dispatch, history }) {
            return history.listen(({ pathname, search }) => {
                if (pathname === '/deviceUpdateDesign') {
                    // 初始化state
                    dispatch({
                        type : 'updateState',
                        payload : {
    ...
    effects: {
        ...
        * loadStaffTree({payload}, {call, put, select}) {
            const { userInfo } = yield select(state => state.deviceUpdateDesign);
        ...
        * getEquipmentList({payload}, {call, put, select}) {
            const {searchParams} = yield select(state => state.deviceUpdateDesign.equipmentSelect);
            let {
                newItem
            } = yield select(({deviceUpdateDesign}) => deviceUpdateDesign);
        ...
        *getList({ payload = {} }, { call, put, select }) {
           ...
           pageSize = 'pageSize' in payload ? payload.pageSize : pageSize;
            ...
            const { data } = yield call(deviceUpdateProgramService.getList, params);
        ...
        *save({ payload = {} },{select,call,put}){
            ...
            const { newItem, structureList, structuresTotal, equipmentSelectTotal } = yield select( ({deviceUpdateDesign}) => deviceUpdateDesign );
            ...
            let { data } = yield call(deviceUpdateProgramService.save, params);
            if(!!data && data.result == 0){
        ...
        *update({ payload = {} },{select,call,put}){
            ...
            let { data } = yield call(deviceUpdateProgramService.update, params);
            if(!!data && data.result == 0){
                yield put({type:'getList'});
