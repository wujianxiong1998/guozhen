import { alarmNoticeMulService, deleteService } from "../services/alarmService";
...
export default {
    ...
    effects: {
        ...
        * loadStaffTree({ payload }, { call, put, select }) {
            const { userInfo } = yield select(state => state.alarmNoticeMul);
        ...
        *getList({ payload = {} }, { call, put, select }) {
            ...
            } = yield select(({alarmNoticeMul}) => alarmNoticeMul);
           ...
           pageSize = 'pageSize' in payload ? payload.pageSize : pageSize;
            ...
            const { data } = yield call(alarmNoticeMulService.getList, params);
        ...
        *save(action,{select,call,put}){
            ...
            const { newItem } = yield select( ({alarmNoticeMul}) => alarmNoticeMul );
            ...
            let { data } = yield call(alarmNoticeMulService.save, params);
            if(!!data && data.result == 0){
        ...
        *update(action,{select,call,put}){
            ...
            let { data } = yield call(alarmNoticeMulService.update, params);
            if(!!data && !data.result){
                // yield put({type:'initParams'});
                yield put({type:'getList'});
        ...
        *getD({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(alarmNoticeMulService.detail, params);
            if(!!data && !data.result){
                yield put({
                    type: 'updateState',
                    payload: {
