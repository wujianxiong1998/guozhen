...
import {pageList, addSave, addUpdate, deleteEntity} from '../services/newsManage';
import {deleteMessage} from "../utils/util";

moment.locale('zh-cn');

export default {
    namespace: 'newsManageM',
    
    subscriptions: {
        setup({dispatch, history}) {
            return history.listen(({pathname}) => {
                if (pathname === '/newsManage') {
    ...
    effects: {
        ...
        * pageList({payload}, {call, put, select}) {
            const {searchParams} = yield select(state => state.newsManageM);
        ...
        * addUpdate({payload}, {call, put, select}) {
            const {detail} = yield select(state => state.newsManageM.modalParams);
