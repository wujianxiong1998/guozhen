import {
    deviceScrapService, deviceChangeService, wheatherAdminByUserIdService
...
export default {
    ...
    effects: {
        ...
        * getEquipmentList({payload}, {call, put, select}) {
            const {searchParams} = yield select(state => state.deviceScrap.equipmentSelect);
            let {
                newItem
            } = yield select(({deviceScrap}) => deviceScrap);
        ...
        * loadStaffTree({payload}, {call, put, select}) {
            const { userInfo } = yield select(state => state.deviceScrap);
        ...
        *getList({ payload = {} }, { call, put, select }) {
           ...
           pageSize = 'pageSize' in payload ? payload.pageSize : pageSize;
            ...
            const { data } = yield call(deviceScrapService.getList, params);
        ...
        *save(action,{select,call,put}){
            ...
            const { newItem, newStructureListSel, userInfo } = yield select( ({deviceScrap}) => deviceScrap );
            ...
            let { data } = yield call(deviceScrapService.save, params);
            if(!!data && data.result == 0){
        ...
        *ignore({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(deviceScrapService.ignore, params);
            if(!!data && !data.result){
                yield put({type:'getList'});
                payload.onSuccess();
            }
        ...
        *getD({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(deviceScrapService.detail, params);
            if(!!data && !data.result){
                yield put({
                    type: 'updateState',
                    payload: {
