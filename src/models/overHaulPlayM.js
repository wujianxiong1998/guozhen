...
import {
    overHaulPlayService, deleteService
...
export default {
    ...
    effects: {
        ...
        * loadStaffTree({payload}, {call, put, select}) {
            const { userInfo } = yield select(state => state.overHaulPlay);
        ...
        * getUserDetail({payload}, {call, put, select}) {
            const {missionItem} = yield select(state => state.overHaulPlay.equipmentSelect);
        ...
        * getEquipmentList({payload}, {call, put, select}) {
            const {searchParams} = yield select(state => state.overHaulPlay.equipmentSelect);
            let {
                newItem
            } = yield select(({overHaulPlay}) => overHaulPlay);
        ...
        *getList({ payload = {} }, { call, put, select }) {
           ...
           pageSize = 'pageSize' in payload ? payload.pageSize : pageSize;
            ...
            const { data } = yield call(overHaulPlayService.getList, params);
        ...
        *save(action,{select,call,put}){
            ...
            const { newItem, structureList, structuresTotal, equipmentSelectTotal } = yield select( ({overHaulPlay}) => overHaulPlay );
            ...
            let { data } = yield call(overHaulPlayService.save, params);
            if(!!data && data.result == 0){
        ...
        *update(action,{select,call,put}){
            ...
            let { data } = yield call(overHaulPlayService.update, params);
            if(!!data && data.result == 0){
                // yield put({type:'initParams'});
                yield put({type:'getList'});
        ...
        //     yield put({ type : 'updateState', payload : {modelLonding : true} });
        //     const { missionItem } = yield select( ({overHaulPlay}) => overHaulPlay );
        ...
        //         repareMan
        //     };
        //     let { data } = yield call(overHaulPlayService.publish, params);
        //     if(!!data && data.result == 0){
        //         // yield put({type:'initParams'});
        //         yield put({type:'getList'});
        ...
        *publish({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(overHaulPlayService.publish, params);
            if(!!data && !data.result){
                yield put({type:'getList'});
                payload.onSuccess();
            }
        ...
        *ignore({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(overHaulPlayService.ignore, params);
            if(!!data && !data.result){
                yield put({type:'getList'});
                payload.onSuccess();
            }
        ...
        *getD({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(overHaulPlayService.detail, params);
            if(!!data && !data.result){
                yield put({
                    type: 'updateState',
                    payload: {
