...
import {
    deviceUpdatePlayService, deleteService, wheatherAdminByUserIdService
...
export default {
    ...
    effects: {
        ...
        * loadStaffTree({payload}, {call, put, select}) {
            const { userInfo } = yield select(state => state.deviceUpdatePlay);
        ...
        * getUserDetail({payload}, {call, put, select}) {
            const {missionItem} = yield select(state => state.deviceUpdatePlay.equipmentSelect);
        ...
        * getEquipmentList({payload}, {call, put, select}) {
            const {searchParams} = yield select(state => state.deviceUpdatePlay.equipmentSelect);
            let {
                newItem
            } = yield select(({deviceUpdatePlay}) => deviceUpdatePlay);
        ...
        *getList({ payload = {} }, { call, put, select }) {
           ...
           pageSize = 'pageSize' in payload ? payload.pageSize : pageSize;
            ...
            const { data } = yield call(deviceUpdatePlayService.getList, params);
        ...
        *save(action,{select,call,put}){
            ...
            const { newItem, structureList, structuresTotal, equipmentSelectTotal } = yield select( ({deviceUpdatePlay}) => deviceUpdatePlay );
            ...
            let { data } = yield call(deviceUpdatePlayService.save, params);
            if(!!data && data.result == 0){
        ...
        *update(action,{select,call,put}){
            ...
            let { data } = yield call(deviceUpdatePlayService.update, params);
            if(!!data && data.result == 0){
                // yield put({type:'initParams'});
                yield put({type:'getList'});
        ...
        //     yield put({ type : 'updateState', payload : {modelLonding : true} });
        //     const { missionItem } = yield select( ({deviceUpdatePlay}) => deviceUpdatePlay );
        ...
        //         repareMan
        //     };
        //     let { data } = yield call(deviceUpdatePlayService.publish, params);
        //     if(!!data && data.result == 0){
        //         // yield put({type:'initParams'});
        //         yield put({type:'getList'});
        ...
        *publish({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(deviceUpdatePlayService.publish, params);
            if(!!data && !data.result){
                yield put({type:'getList'});
                payload.onSuccess();
            }
        ...
        *ignore({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(deviceUpdatePlayService.ignore, params);
            if(!!data && !data.result){
                yield put({type:'getList'});
                payload.onSuccess();
            }
        ...
        *getD({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(deviceUpdatePlayService.detail, params);
            if(!!data && !data.result){
                yield put({
                    type: 'updateState',
                    payload: {
