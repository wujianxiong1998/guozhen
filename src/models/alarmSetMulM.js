import { alarmSetMulService, deleteService } from "../services/alarmService";
...
export default {
    ...
    effects: {
        ...
        * loadStaffTree({payload}, {call, put, select}) {
            const { userInfo } = yield select(state => state.alarmSetMul);
        ...
        *getList({ payload = {} }, { call, put, select }) {
            ...
            } = yield select(({alarmSetMul}) => alarmSetMul);
           ...
           pageSize = 'pageSize' in payload ? payload.pageSize : pageSize;
            ...
            const { data } = yield call(alarmSetMulService.getList, params);
        ...
        *save(action,{select,call,put}){
            ...
            const { newItem } = yield select( ({alarmSetMul}) => alarmSetMul );
            ...
            let { data } = yield call(alarmSetMulService.save, params);
            if(!!data && data.result == 0){
        ...
        *update(action,{select,call,put}){
            ...
            let { data } = yield call(alarmSetMulService.update, params);
            if(!!data && !data.result){
                // yield put({type:'initParams'});
                yield put({type:'getList'});
        ...
        *getD({ payload }, { call, put, select }) {
            ...
            const { data } = yield call(alarmSetMulService.detail, params);
            if(!!data && !data.result){
                yield put({
                    type: 'updateState',
                    payload: {
