import { getFactoryFileList,addFactoryFile,commonDelete,updateFactoryFile,
    loadCommonParamSelect,loadBusinessUnitSelect,loadRegionalCompanySelect,loadWaterFactorySelect } from '../services/remoteData';
...
export default {
    ...
    effects : {
            ...
            if(!!data && !data.result) {
                if('data' in data && Array.isArray(data.data)) {
                    yield put({
                        type : 'updateState',
                        payload : {
                            waterFactoryNameSelect : data.data
                        }
                    })
                }
            }
        ...
        *getList({ payload = {} }, { call, put, select }) {
            ...
            } = yield select(({factoryFile}) => factoryFile);
            ...
            const { data } = yield call(getFactoryFileList, VtxUtil.submitTrim(params));
        ...
        *saveOrUpdate({ payload }, { call, put, select }) {
            ...
            const { newItem, editItem } = yield select( ({factoryFile}) => factoryFile );
            const {
                acceptance, acceptanceTime, businessUnitId, businessUnitName, constructionTime,constructionUnit,
                designUnit, envAssessment, id, investment, operationTime, regionalCompanyId, regionalCompanyName,
                research, waterFactoryId, waterFactoryName
            } = payload.btnType === 'add' ? newItem : editItem;
            let params = {
                acceptance, acceptanceTime, businessUnitId, businessUnitName, constructionTime,constructionUnit,
                designUnit, envAssessment, id, investment, operationTime, regionalCompanyId, regionalCompanyName,
                research,waterFactoryId,
                tenantId: VtxUtil.getUrlParam('tenantId')
            };
            
            const { data } = yield call( payload.btnType === 'add' ? 
                    addFactoryFile : updateFactoryFile, VtxUtil.submitTrim(params));
