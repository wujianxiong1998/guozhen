import { getWaterFactoryList,addWaterFactory,commonDelete,updateWaterFactory,
    loadCommonParamSelect,loadBusinessUnitSelect,loadRegionalCompanySelect } from '../services/remoteData';
...
export default {
    ...
    effects : {
        ...
        *getList({ payload = {} }, { call, put, select }) {
            ...
            } = yield select(({waterFactory}) => waterFactory);
            ...
            const { data } = yield call(getWaterFactoryList, VtxUtil.submitTrim(params));
        ...
        *saveOrUpdate({ payload }, { call, put, select }) {
            ...
            const { newItem, editItem } = yield select( ({waterFactory}) => waterFactory );
            const {
                id, name, floorArea, businessId, processTypeId, productTypeId, startDate, businessUnitId,
                regionalCompanyId,endDate, processSize, waterStandard, minWaterFlow, waterPrice,longitude,latitude
            ...
            const { data } = yield call( payload.btnType === 'add' ? 
                    addWaterFactory : updateWaterFactory, VtxUtil.submitTrim(params));
