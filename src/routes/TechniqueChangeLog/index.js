...
function TechniqueChangeLog({ dispatch, techniqueChangeLog, accessControlM  }) {

    const { queryParams, viewItem, getData, fileListVersion,
        currentPage, pageSize, loading, dataSource, total,
    } = techniqueChangeLog;
    let buttonLimit = {};
    if (accessControlM['techniqueChangeLog'.toLowerCase()]) {
        buttonLimit = accessControlM['techniqueChangeLog'.toLowerCase()];
    }
    const ParamsUpdateState = (obj) => {
        dispatch({
            type : 'techniqueChangeLog/updateState',
            payload : {
                queryParams: {
                    ...queryParams,
                    ...obj
    ...
    // 更新表格数据
	const getList = () => {
        dispatch({type: 'techniqueChangeLog/updateState', payload: { currentPage: 1, pageSize: 10 }})
		dispatch({type : 'techniqueChangeLog/getList'});
	}
	...
	const vtxGridParams = {
		...
		clear() {
			dispatch({type : 'techniqueChangeLog/initQueryParams'});
			dispatch({type : 'techniqueChangeLog/getList'});
		}
    };
	...
	let vtxDatagridProps = {
        ...
        onChange(pagination, filters, sorter){
            dispatch({
            	type:'techniqueChangeLog/getList',
            	payload : {
...
export default connect(({ techniqueChangeLog, accessControlM }) => ({ techniqueChangeLog, accessControlM }))(TechniqueChangeLog);
