...
import NewItem from '../../components/basicInformation/Add';
import EditItem from '../../components/basicInformation/Add';
import ViewItem from '../../components/basicInformation/View';
import styles from './index.less';
import { handleColumns } from '../../utils/tools';

function BasicInformation({ dispatch, basicInformation, common, accessControlM }) {
    const {
        searchParams,
    ...
    const updateState = (obj) => {
        dispatch({
            type: 'basicInformation/updateState',
            payload: {
                ...obj
            }
        })
    ...
    // 更新表格数据
    const getList = () => {
        dispatch({ type: 'basicInformation/updateQueryParams' });
        dispatch({ type: 'basicInformation/getList' });
    }
    ...
    const vtxGridParams = {
        ...
        clear() {
            dispatch({ type: 'basicInformation/initQueryParams' });
            dispatch({ type: 'basicInformation/getList' });
        }
    };
        ...
        ['操作', 'action', {
            renderButtons: () => {
                ...
                btns.push({
                    ...
                    onClick(rowData) {
                        dispatch({
                            type: 'basicInformation/deleteItems', payload: {
                                ids: [rowData.id],
    ...
    const updateNewWindow = (status = true) => {
        ...
        if (!status) {
            dispatch({ type: 'basicInformation/initNewItem' });
        }
    }
    const newItemProps = {
        ...
        contentProps: {
            ...
            save() {
                dispatch({
                    type: 'basicInformation/saveOrUpdate', payload: {
                        btnType: 'add',
                        onSuccess: function () {
...
export default connect(
    ({ basicInformation, common, accessControlM }) => ({ basicInformation, common, accessControlM })
)(BasicInformation);