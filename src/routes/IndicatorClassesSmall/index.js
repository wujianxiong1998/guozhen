...
function IndicatorClassesSmall({ dispatch, indicatorClassesSmall, accessControlM }) {
    const {
        searchParams,
        currentPage, pageSize, loading, dataSource, total, selectedRowKeys,
        newItem, editItem, viewItem
    } = indicatorClassesSmall;
    let buttonLimit = {};
    ...
    const updateState = (obj) => {
        dispatch({
            type: 'indicatorClassesSmall/updateState',
            payload: {
                ...obj
            }
        })
    ...
    // 更新表格数据
    const getList = () => {
        dispatch({ type: 'indicatorClassesSmall/updateQueryParams' });
        dispatch({ type: 'indicatorClassesSmall/getList' });
    }
    ...
    const vtxGridParams = {
        ...
        clear() {
            dispatch({ type: 'indicatorClassesSmall/initQueryParams' });
            dispatch({ type: 'indicatorClassesSmall/getList' });
        }
    };
        ...
        ['操作', 'action', {
            renderButtons: () => {
                ...
                btns.push({
                    ...
                    onClick(rowData) {
                        dispatch({
                            type: 'indicatorClassesSmall/deleteItems', payload: {
                                ids: [rowData.id],
                                onSuccess: function (ids) {
                                    let page = currentPage != 1 && ids.length === (total - (currentPage - 1) * pageSize) ?
                                        currentPage - 1 : currentPage;
                                    dispatch({
                                        type: 'indicatorClassesSmall/getList',
                                        payload: {
                                            selectedRowKeys: [],
                                            currentPage: page
                                        }
    ...
    const updateNewWindow = (status = true) => {
        ...
        if (!status) {
            dispatch({ type: 'indicatorClassesSmall/initNewItem' });
        }
    }
    const newItemProps = {
        ...
        contentProps: {
            ...
            save() {
                dispatch({
                    type: 'indicatorClassesSmall/saveOrUpdate', payload: {
                        btnType: 'add',
                        onSuccess: function () {
...
export default connect(
    ({ indicatorClassesSmall, accessControlM }) => ({ indicatorClassesSmall,accessControlM })
)(IndicatorClassesSmall);