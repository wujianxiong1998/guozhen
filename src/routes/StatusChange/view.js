...
class ViewItem extends React.Component {
    ...
    render() {
        ...
        let { id, code, name, typeName, modelNum, applyDay, structuresName, deviceStatusStr, newStructuresName, newDeviceStatusStr,
            auditPeopleName, auditMemo, changeReason,
        } = contentProps;
        let modalProps = {
            title: title,
            visible: visible,
        ...
        return (
            <VtxModal {...modalProps}>
                ...
                <div data-modallist={{layout:{width:50,name:'设备编号',type:'text'}}}>{code}</div>
                    ...
                    <div data-modallist={{layout:{width:100,name:'变更原因',type:'text'}}}>{changeReason}</div>
                    <div data-modallist={{layout:{width:100,name:'审核意见',type:'text'}}}>{auditMemo}</div>
                </VtxModalList>   
            </VtxModal>
        )
    }
