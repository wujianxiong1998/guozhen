...
class ProductionFill extends React.Component{
    constructor(props){
        super(props)
    }
    componentDidMount(){
        const {dispatch}  = this.props;        
        dispatch({ type: 'productionFill/loadWaterFactorySelect' }).then(()=>{
            dispatch({ type: 'productionFill/getList' });
        });
    }
    //保存
    handleSave=()=>{
        this.props.dispatch({ type: "productionFill/calculateTargetValue" }).then(()=>{
            this.props.dispatch({
                type: 'productionFill/saveOrUpdate',
                payload: {
                    dataStatus: 0,
                    onSuccess: () => {
    ...
    jumpToHistory=()=>{
        // location.hash = location.hash.replace('productionFill', 'productionHistory')
        this.props.history.push(`/productionHistory?tenantId=${VtxUtil.getUrlParam('tenantId')}&userId=${VtxUtil.getUrlParam('userId')}&token=${VtxUtil.getUrlParam('token')}&hasBack=1`);
    }
    render(){
        const {dispatch} = this.props;
        const {
        ...
        } = this.props.productionFill;

        const updateState = (obj) => {
            dispatch({
                type: 'productionFill/updateState',
                payload: {
                    ...obj
                }
            })
        ...
        // 更新表格数据
        const getList = () => {
            dispatch({ type: 'productionFill/updateQueryParams' });
            dispatch({ type: 'productionFill/getList' });
        }
        ...
        const vtxGridParams = {
            ...
            clear() {
                dispatch({ type: 'productionFill/initQueryParams' });
                dispatch({ type: 'productionFill/getList' });
            }
        };
            ...
            ['值', 'dataValue', {
                render: (text, record, index) => {
                    if (record.categoryKey ==='primitiveTarget'){
                        return <Input onBlur={() => { dispatch({ type:"productionFill/calculateTargetValue"})}} value={text} onChange={e => {
                            updateState({
                                dataSource: {
                                    [index]: {
            ...
            <div className={styles.normal}>
                ...
                <div className={styles.normal_body}>
                    ...
                    <div className={styles.tableHeader}>
                        {searchParams.dateValue+waterFactoryName+'生产数据填报'}
                    </div>
