...
function AlarmNoticeMul({ dispatch, alarmNoticeMul, accessControlM }) {
    ...
    let buttonLimit = {};
    if (accessControlM['alarmNoticeMul'.toLowerCase()]) {
        buttonLimit = accessControlM['alarmNoticeMul'.toLowerCase()];
    }
    const ParamsUpdateState = (obj) => {
        dispatch({
            type : 'alarmNoticeMul/updateState',
            payload : {
                queryParams: {
                    ...queryParams,
                    ...obj
	...
	const getList = () => {
		...
		dispatch({type : 'alarmNoticeMul/getList'});
	}
	...
	const vtxGridParams = {
		...
		clear() {
			dispatch({type : 'alarmNoticeMul/initQueryParams'});
			dispatch({type : 'alarmNoticeMul/getList'});
		}
    };
        ...
        ['操作', 'action', { renderButtons : (text, record) => {
        	...
        	btns.push({
                name: <Icon type='delete'
                    title='删除' />,
        		onClick(rowData) {
                    dispatch({type:'alarmNoticeMul/delete',payload: {
                        ids: rowData.id,
                        onSuccess: function(){
                            updateState({
                                selectedRowKeys : []
	...
	const showNewWindow = () => {
		dispatch({
    		type : 'alarmNoticeMul/updateNewItem',
    		payload : {
    			visible : true
    		}
    	});
    ...
    const updateNewWindow = (status = true) => {
    	if(!status) {
    		dispatch({ type : 'alarmNoticeMul/initParams' });
    	}
    	dispatch({
    		type : 'alarmNoticeMul/updateNewItem',
    		payload : {
    ...
    const newItemProps = {
        ...
        contentProps:{
            ...
            onSave() {
                dispatch({
                    type:'alarmNoticeMul/save'
                }).then(() => updateNewWindow(false));
            },
            onUpdate() {
                dispatch({
                    type:'alarmNoticeMul/update'
                }).then(() => updateNewWindow(false));
            },
...
export default connect(({ alarmNoticeMul, accessControlM }) => ({ alarmNoticeMul, accessControlM }))(AlarmNoticeMul);