...
function IndicatorUnit({ dispatch, indicatorUnit, accessControlM }) {
    const {
        searchParams,
    ...
    const updateState = (obj) => {
        dispatch({
            type: 'indicatorUnit/updateState',
            payload: {
                ...obj
            }
        })
    ...
    // 更新表格数据
    const getList = () => {
        dispatch({ type: 'indicatorUnit/updateQueryParams' });
        dispatch({ type: 'indicatorUnit/getList' });
    }
    ...
    const vtxGridParams = {
        ...
        clear() {
            dispatch({ type: 'indicatorUnit/initQueryParams' });
            dispatch({ type: 'indicatorUnit/getList' });
        }
    };
    ...
    const updateNewWindow = (status = true) => {
        ...
        if (!status) {
            dispatch({ type: 'indicatorUnit/initNewItem' });
        }
    }
    const newItemProps = {
        ...
        contentProps: {
            ...
            save() {
                dispatch({
                    type: 'indicatorUnit/saveOrUpdate', payload: {
                        btnType: 'add',
                        onSuccess: function () {
...
export default connect(
    ({ indicatorUnit, accessControlM }) => ({ indicatorUnit, accessControlM })
)(IndicatorUnit);