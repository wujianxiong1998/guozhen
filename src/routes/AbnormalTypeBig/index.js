...
function AbnormalTypeBig({ dispatch, abnormalTypeBig, accessControlM  }) {
    const {
        searchParams,
    ...
    let buttonLimit = {};
    if (accessControlM['abnormalTypeBig'.toLowerCase()]) {
        buttonLimit = accessControlM['abnormalTypeBig'.toLowerCase()];
    }
    const updateState = (obj) => {
        dispatch({
            type: 'abnormalTypeBig/updateState',
            payload: {
                ...obj
            }
        })
    ...
    // 更新表格数据
    const getList = () => {
        dispatch({ type: 'abnormalTypeBig/updateQueryParams' });
        dispatch({ type: 'abnormalTypeBig/getList' });
    }
    ...
    const vtxGridParams = {
        ...
        clear() {
            dispatch({ type: 'abnormalTypeBig/initQueryParams' });
            dispatch({ type: 'abnormalTypeBig/getList' });
        }
    };
    ...
    const updateNewWindow = (status = true) => {
        ...
        if (!status) {
            dispatch({ type: 'abnormalTypeBig/initNewItem' });
        }
    }
    const newItemProps = {
        ...
        contentProps: {
            ...
            save() {
                dispatch({
                    type: 'abnormalTypeBig/saveOrUpdate', payload: {
                        btnType: 'add',
                        onSuccess: function () {
...
export default connect(
    ({ abnormalTypeBig, accessControlM }) => ({ abnormalTypeBig, accessControlM  })
)(AbnormalTypeBig);