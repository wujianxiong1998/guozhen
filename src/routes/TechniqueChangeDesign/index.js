...
function TechniqueChangeDesign({ dispatch, techniqueChangeDesign, accessControlM}) {
    ...
    } = techniqueChangeDesign;
    let buttonLimit = {};
    if (accessControlM['techniqueChangeDesign'.toLowerCase()]) {
        buttonLimit = accessControlM['techniqueChangeDesign'.toLowerCase()];
    }
    const ParamsUpdateState = (obj) => {
        dispatch({
            type : 'techniqueChangeDesign/updateState',
            payload : {
                queryParams: {
                    ...queryParams,
                    ...obj
    ...
    // 更新表格数据
	const getList = () => {
        dispatch({type: 'techniqueChangeDesign/updateState', payload: { currentPage: 1, pageSize: 10 }})
		dispatch({type : 'techniqueChangeDesign/getList'});
	}
	...
	const vtxGridParams = {
		...
		clear() {
			dispatch({type : 'techniqueChangeDesign/initQueryParams'});
			dispatch({type : 'techniqueChangeDesign/getList'});
		}
    };
	...
	const columns = [
        ...
        ['操作', 'action', { renderButtons : (text, record) => {
            ...
            btns.push({
        		...
        		onClick(rowData) {
					// dispatch({
                    //     type : 'techniqueChangeDesign/getD',
                    //     payload : {
                    //         id: rowData.id
                    //     }
                    // }).then(() => {
            ...
            if (record.dataStatus === 'waitPublish') {
                ...
                btns.push({
                    name: <Icon type='delete'
                        title='删除' />,
                    onClick(rowData) {
                        dispatch({type:'techniqueChangeDesign/delete',payload: {
                            ids: rowData.id,
                            onSuccess: function(){
                                updateState({
                                    selectedRowKeys : []
	...
	let vtxDatagridProps = {
        ...
        onChange(pagination, filters, sorter){
            dispatch({
            	type:'techniqueChangeDesign/getList',
            	payload : {
	...
	const showNewWindow = () => {
		dispatch({
    		type : 'techniqueChangeDesign/updateNewItem',
    		payload : {
    			visible : true
    		}
    	});
    ...
    const updateNewWindow = (status = true) => {
    	if(!status) {
    		dispatch({ type : 'techniqueChangeDesign/initParams' });
    	}
    	dispatch({
    		type : 'techniqueChangeDesign/updateNewItem',
    		payload : {
    ...
    const clearEquipmentList = () => {
        dispatch({type: 'techniqueChangeDesign/clearEquipmentSearchParams'});
        dispatch({type: 'techniqueChangeDesign/getEquipmentList'});
        dispatch({
            type : 'techniqueChangeDesign/updateNewItem',
            payload : {
                deviceId: '',
    ...
    const getEquipmentList = (name, page, size) => {
        dispatch({type: 'techniqueChangeDesign/updateEquipmentSearchParams', payload: {name, page, size}});
        dispatch({type: 'techniqueChangeDesign/getEquipmentList'});
    };
    const newItemProps = {
        ...
        contentProps:{
            ...
            onSave(e) {
                dispatch({
                    type:'techniqueChangeDesign/save',
                    payload: {
                        saveOrSubmit: e
                    }
            ...
            onUpdate(e) {
                dispatch({
                    type:'techniqueChangeDesign/update',
                    payload: {
                        saveOrSubmit: e
                    }
                }).then(() => updateNewWindow(false));
...
export default connect(({ techniqueChangeDesign, accessControlM }) => ({ techniqueChangeDesign, accessControlM }))(TechniqueChangeDesign);
