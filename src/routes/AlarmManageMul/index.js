...
function AlarmManageMul({ dispatch, alarmManageMul, accessControlM}) {
    ...
    let buttonLimit = {};
    if (accessControlM['alarmManageMul'.toLowerCase()]) {
        buttonLimit = accessControlM['alarmManageMul'.toLowerCase()];
    }
    const ParamsUpdateState = (obj) => {
        dispatch({
            type : 'alarmManageMul/updateState',
            payload : {
                queryParams: {
                    ...queryParams,
                    ...obj
	...
	const getList = () => {
		...
		dispatch({type : 'alarmManageMul/getList'});
	}
	...
	const vtxGridParams = {
		...
		clear() {
			dispatch({type : 'alarmManageMul/initQueryParams'});
			dispatch({type : 'alarmManageMul/getList'});
		}
    };
        ...
        ['操作', 'action', { renderButtons : (text, record) => {
        	...
        	btns.push({
                name: <Icon type='delete'
                    title='解除' />,
        		onClick(rowData) {
                    dispatch({type:'alarmManageMul/delete',payload: {
                        ids: rowData.id,
                        onSuccess: function(){
                            updateState({
                                selectedRowKeys : []
	...
	const showNewWindow = () => {
		dispatch({
    		type : 'alarmManageMul/updateNewItem',
    		payload : {
    			visible : true
    		}
    	});
    ...
    const updateNewWindow = (status = true) => {
    	if(!status) {
    		dispatch({ type : 'alarmManageMul/initParams' });
    	}
    	dispatch({
    		type : 'alarmManageMul/updateNewItem',
    		payload : {
    ...
    const newItemProps = {
        ...
        contentProps:{
            ...
            onSave() {
                dispatch({
                    type:'alarmManageMul/save'
                }).then(() => updateNewWindow(false));
            },
            onUpdate() {
                dispatch({
                    type:'alarmManageMul/update'
                }).then(() => updateNewWindow(false));
            },
...
export default connect(({ alarmManageMul, accessControlM }) => ({ alarmManageMul, accessControlM }))(AlarmManageMul);