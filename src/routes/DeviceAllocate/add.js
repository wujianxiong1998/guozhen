...
class AddItem extends React.Component {
    ...
    render() {
        let { 
            modalPropsItem, contentProps, structureList, userList, maintainType, equipmentStatus, newStructureListSel, newWaterFactorySel,
        ...
        let { id, structuresId, newStructuresId, structuresName, deviceId, name, code, deviceStatus, newDeviceStatus, allocationReason, auditPeopleId, auditPeopleName,
            waterFactoryName, newWaterFactoryId,
        ...
        return (
            <VtxModal {...modalProps}>
                <VtxModalList {...modallistProps} ref={(lis) => this.lis1 = lis}>
                        ...
                        selectRow={
                            (rows) => {
                                updateItem({
                                    ...
                                    waterFactoryId: rows.waterFactoryId,
                                    waterFactoryName: rows.waterFactoryName,
                                }); 
                            }
                        }
                        tableCfg={{
                    ...
                    <Input
                        data-modallist={{
                            layout: {width: 50,name: '原水厂',require: true},
                            regexp: {
                        ...
                        value={waterFactoryName}
                        disabled
                        onChange={e=>updateItem({waterFactoryName:e.target.value})}
                    />
                    <Select
                        value={newWaterFactoryId}
                        data-modallist={{
                            layout: {width: 50,name: '新水厂',require: true},
                            regexp: {
                                value: newWaterFactoryId
                            }
                        }}
                        onChange={(value)=>{
                            updateItem({
                                newWaterFactoryId: value
                            }); 
                        }}
                    >
                        {
                            newWaterFactorySel.map(item => {
                                return <Option key={item.id}>{item.name}</Option>
                            })
                        }
                    </Select>
                    ...
                    <Input.TextArea
                        data-modallist={{
                            layout: {width: 100,name: '调拨说明',require: true},
                            regexp: {
                                value: allocationReason
                            }
                        }}
                        value={allocationReason}
                        onChange={e=>updateItem({allocationReason:e.target.value})}
                    />
                    <TreeSelect 
                        data-modallist={{
