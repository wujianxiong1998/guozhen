...
class ViewItem extends React.Component {
    ...
    render() {
        ...
        let { id, code, name, typeName, modelNum, applyDay, structuresName, deviceStatusStr, newStructuresName, newDeviceStatusStr,
            auditPeopleName, auditMemo, allocationReason, waterFactoryName, newWaterFactoryName,
        } = contentProps;
        let modalProps = {
            title: title,
            visible: visible,
        ...
        return (
            <VtxModal {...modalProps}>
                ...
                <div data-modallist={{layout:{width:50,name:'设备编号',type:'text'}}}>{code}</div>
                    ...
                    <div data-modallist={{layout:{width:50,name:'原水厂',type:'text'}}}>{waterFactoryName}</div>
                    <div data-modallist={{layout:{width:50,name:'新水厂',type:'text'}}}>{newWaterFactoryName}</div>
                    <div data-modallist={{layout:{width:50,name:'审核人',type:'text'}}}>{auditPeopleName}</div>
                    <div data-modallist={{layout:{width:100,name:'调拨说明',type:'text'}}}>{allocationReason}</div>
                </VtxModalList>   
            </VtxModal>
        )
    }
