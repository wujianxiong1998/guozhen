...
function TechniqueChangePlay({ dispatch, techniqueChangePlay, accessControlM }) {
    ...
    let buttonLimit = {};
    if (accessControlM['techniqueChangePlay'.toLowerCase()]) {
        buttonLimit = accessControlM['techniqueChangePlay'.toLowerCase()];
    }
    const ParamsUpdateState = (obj) => {
        dispatch({
            type : 'techniqueChangePlay/updateState',
            payload : {
                queryParams: {
                    ...queryParams,
                    ...obj
    ...
    // 更新表格数据
	const getList = () => {
        dispatch({type: 'techniqueChangePlay/updateState', payload: { currentPage: 1, pageSize: 10 }});
		dispatch({type : 'techniqueChangePlay/getList'});
	}
	...
	const vtxGridParams = {
		...
		clear() {
			dispatch({type : 'techniqueChangePlay/initQueryParams'});
			dispatch({type : 'techniqueChangePlay/getList'});
		}
    };
            ...
            if (record.dataStatus === 'waitPublish') {
                ...
                btns.push({
                    ...
                    onClick(rowData) {
                        ...
                        Modal.confirm({
                            ...
                            cancelText: '取消',
                            onOk() {
                                dispatch({type:'techniqueChangePlay/publish',payload: {
                                    id: rowData.id,
                ...
                btns.push({
                    name: <Icon type='delete'
                        title='删除' />,
                    onClick(rowData) {
                        dispatch({type:'techniqueChangePlay/delete',payload: {
                            ids: rowData.id,
                            onSuccess: function(){
                                updateState({
                                    selectedRowKeys : []
                ...
                btns.push({
                    ...
                    onClick(rowData) {
                        Modal.confirm({
                            ...
                            cancelText: '取消',
                            onOk() {
                                dispatch({type:'techniqueChangePlay/ignore',payload: {
                                    id: rowData.id,
                                    onSuccess: function(){
	...
	let vtxDatagridProps = {
        ...
        onChange(pagination, filters, sorter){
            dispatch({
            	type:'techniqueChangePlay/getList',
            	payload : {
	...
	const showNewWindow = () => {
		dispatch({
    		type : 'techniqueChangePlay/updateNewItem',
    		payload : {
    			visible : true
    		}
    	});
    ...
    const updateNewWindow = (status = true) => {
    	if(!status) {
    		dispatch({ type : 'techniqueChangePlay/initParams' });
    	}
    	dispatch({
    		type : 'techniqueChangePlay/updateNewItem',
    		payload : {
    ...
    const clearEquipmentList = () => {
        dispatch({type: 'techniqueChangePlay/clearEquipmentSearchParams'});
        dispatch({type: 'techniqueChangePlay/getEquipmentList'});
        dispatch({
            type : 'techniqueChangePlay/updateNewItem',
            payload : {
                deviceId: '',
    ...
    const getEquipmentList = (name, page, size) => {
        dispatch({type: 'techniqueChangePlay/updateEquipmentSearchParams', payload: {name, page, size}});
        dispatch({type: 'techniqueChangePlay/getEquipmentList'});
    };
    const newItemProps = {
        ...
        contentProps:{
            ...
            onSave() {
                dispatch({
                    type:'techniqueChangePlay/save'
                }).then(() => updateNewWindow(false));
            },
            onUpdate() {
                dispatch({
                    type:'techniqueChangePlay/update'
                }).then(() => updateNewWindow(false));
            },
	...
	const showMissionWindow = () => {
		dispatch({
    		type : 'techniqueChangePlay/updateMissionItem',
    		payload : {
    			visible : true
    		}
    	});
    ...
    const missionItemProps = {
        ...
        contentProps:{
            ...
            onPublish() {
                dispatch({
                    type:'techniqueChangePlay/publish'
...
export default connect(({ techniqueChangePlay, accessControlM }) => ({ techniqueChangePlay, accessControlM  }))(TechniqueChangePlay);
