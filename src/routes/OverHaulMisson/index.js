...
function OverHaulMisson({ dispatch, overHaulMisson, accessControlM  }) {

    const { queryParams, newItem, structureList, viewItem, updataItem, getData, fileListVersion, userList, checkItem,
        equipmentSelectList, equipmentSelectTotal,
        currentPage, pageSize, loading, dataSource, total, selectedRowKeys, modelLonding,
        partDataSource, partDataSourceCopy, partParams,
    } = overHaulMisson;
    let buttonLimit = {};
    if (accessControlM['overHaulMission'.toLowerCase()]) {
        buttonLimit = accessControlM['overHaulMission'.toLowerCase()];
    }
    
    const ParamsUpdateState = (obj) => {
        dispatch({
            type : 'overHaulMisson/updateState',
            payload : {
                queryParams: {
                    ...queryParams,
                    ...obj
    ...
    // 更新表格数据
	const getList = () => {
        dispatch({type: 'overHaulMisson/updateState', payload: { currentPage: 1, pageSize: 10 }})
		dispatch({type : 'overHaulMisson/getList'});
	}
	...
	const vtxGridParams = {
		...
		clear() {
			dispatch({type : 'overHaulMisson/initQueryParams'});
			dispatch({type : 'overHaulMisson/getList'});
		}
    };
        ...
        ['操作', 'action', { renderButtons : (text, record) => {
            ...
            if (record.dataStatus === 'onAudit' && buttonLimit['AUDIT']) {
                btns.push({
                    ...
                    onClick(rowData) {
                        dispatch({
                            type : 'overHaulMisson/checkItemParams',
                            payload : {
                                visible: true,
                                id: rowData.id,
                            }
	...
	const showNewWindow = () => {
		dispatch({
    		type : 'overHaulMisson/updateNewItem',
    		payload : {
    			visible : true
    		}
    	});
    ...
    const updateNewWindow = (status = true) => {
    	if(!status) {
    		dispatch({ type : 'overHaulMisson/initParams' });
    	}
    	dispatch({
    		type : 'overHaulMisson/updateNewItem',
    		payload : {
    ...
    const clearEquipmentList = () => {
        dispatch({type: 'overHaulMisson/clearEquipmentSearchParams'});
        dispatch({type: 'overHaulMisson/getEquipmentList'});
        dispatch({
            type : 'overHaulMisson/updateNewItem',
            payload : {
                deviceId: '',
    ...
    const getEquipmentList = (name, page, size) => {
        dispatch({type: 'overHaulMisson/updateEquipmentSearchParams', payload: {name, page, size}});
        dispatch({type: 'overHaulMisson/getEquipmentList'});
    };

    const updatePartParams = (target, value) => {
        dispatch({
            type: 'overHaulMisson/updatePartParams',
            payload: {
                [target]: value
    ...
    const newItemProps = {
        ...
        contentProps:{
            ...
            getSparePartsList() {
                dispatch({type: 'overHaulMisson/getSparePartsList'})
            },

            openModal() {
                dispatch({type: 'overHaulMisson/getSparePartsList'}).then(() => {
                    dispatch({
                        type: 'overHaulMisson/updatePartParams',
                        payload: {
                            modalVisible: true
                ...
                // dispatch({type: 'overHaulMisson/getPartList'})
            },
    ...
    const deleteItems = () => {
    	Modal.confirm({
            ...
            cancelText: '取消',
            onOk() {
                dispatch({type:'overHaulMisson/delete',payload: {
...
export default connect(({ overHaulMisson, accessControlM }) => ({ overHaulMisson, accessControlM  }))(OverHaulMisson);
